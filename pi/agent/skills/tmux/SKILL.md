...
- **Never use `-L` as a shorthand for `-S`** — the correct flag for a custom socket path is `-S "$SOCKET"` throughout.
...
Special rules when the spawned process is another `pi` instance:

- **Resolve pi to an absolute path before starting tmux** — tmux sessions start non-login shells and
  the `pi` command may be absent from the stripped PATH. Use:
  ```bash
  PI="$(command -v pi 2>/dev/null || true)"
  [ -x "$PI" ] || { echo "pi executable not found; install pi or ensure it is on PATH" >&2; exit 1; }
  ```
- **Non-interactive flag is `-p`**, not `--yes`. `--yes` does not exist and causes an immediate
  error. The correct invocation is: `pi -p "<prompt>"`
- **Redirect output to a log file** — pi's `-p` mode uses an alternate screen buffer (TUI)
  that clears on exit, making `capture-pane` see nothing. Always redirect:
  ```bash
  LOG="/tmp/pi-delegate-N.log"
  CMD="cd /path/to/project && $PI -p \"\$(cat $PROMPT_FILE)\" > $LOG 2>&1; echo \"AGENT_DONE:\$?\" >> $LOG"
  tmux -S "$SOCKET" send-keys -t session:0.0 -l -- "$CMD"
  tmux -S "$SOCKET" send-keys -t session:0.0 -- "" Enter
  ```
  Note: send the command with `-l` (literal) first, then send `Enter` separately.
- **Poll the sentinel, not `pane_current_command`** — if pi fails to launch, the shell returns
  to idle immediately and `pane_current_command` will show `bash`/`zsh` whether the agent
  succeeded, failed, or never ran. Always poll for `AGENT_DONE:` in the log file instead:
  ```bash
  grep -q 'AGENT_DONE:' "$LOG" && echo done || echo still running
  ```
- **Shell quoting with `$(cat $PROMPT_FILE)`** — wrap the entire launch command in a variable
  and use `-l` literal send-keys (as above). If you inline `"$(cat ...)"` directly inside
  double-quoted send-keys, shell expansion will interpret lines inside the prompt file as
  commands, breaking the invocation.
