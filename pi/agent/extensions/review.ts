...
function setReviewWidget(ctx: ExtensionContext, active: boolean) {
	...
	ctx.ui.setWidget("review", (_tui, theme) => {
		const longMsg = reviewLoopInProgress
		...
		const shortMsg = reviewLoopInProgress ? "review: fixing" : "review · /end-review";
		let text: Text | undefined;
		return {
			render(width: number) {
				// Width-aware: a ~62-char status string starves the footer's gap math
				// and pushes narrow terminals toward overflow. Shorten under 60 cols.
				const message = width < 60 ? shortMsg : longMsg;
				text = new Text(theme.fg("warning", message), 0, 0);
