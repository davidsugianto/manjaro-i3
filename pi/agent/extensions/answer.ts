...
class QnAComponent implements Component {
	...
	render(width: number): string[] {
		...
		const lines: string[] = [];
		// Floor at 8 so boxWidth - 2 never goes negative: String.repeat(negative)
		// throws RangeError and takes down the render loop on tiny/just-resized terminals.
		const boxWidth = Math.max(8, Math.min(width - 4, 120)); // Allow wider box
		...
		// Floor at 1: at boxWidth=8, contentWidth=4 and this would go negative, and
		// editor.render(negative) throws RangeError, crashing the overlay on tiny terminals.
